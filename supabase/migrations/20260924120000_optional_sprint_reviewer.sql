-- Optional reviewer: Complete is allowed without one; pending_review still requires one.
-- Revert: revert/20260924120000_optional_sprint_reviewer.sql
...
begin
  ...
  if new.status = 'pending_review' and new.reviewer_id is null then
    raise exception 'A reviewer is required before a task can move to pending review';
  end if;
  if new.status = 'done'
     and new.reviewer_id is not null
     and coalesce(new.review_approved, false) = false then
