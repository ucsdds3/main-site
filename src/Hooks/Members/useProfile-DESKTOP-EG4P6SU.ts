...
export function useProfile() {
  const signupSchema = z.object({
    ...
    major: z.string().refine((val) => val !== user?.user_metadata.major, {
    ...
    gender: z.string().refine((val) => val !== user?.user_metadata.gender, {
  ...
  const handleUpdateProfile = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: memberError } = await supabase.from("Members").update(data).eq("email", data?.email);
    if (memberError) {
