...
const Stats = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-0 w-auto lg:w-full max-w-6xl px-8 md:px-0 border-l-4 md:border-l-0 items-center justify-items-center">
      {statValues.map((value, index) => (
        <div
          key={index}
          className={`h-30 w-full flex flex-col items-center px-4 ${index > 0 ? "lg:border-l-6" : ""}`}
          ref={(el) => {
        ...
        >
          ...
          >
            {Math.round(value) + (Math.round(value) === stats[index].value ? "+" : "")}
          </span>
