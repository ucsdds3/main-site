...
const BrowserCard = memo(function BrowserCard({ name, description, image, points }: PortalEvent) {
  ...
  return (
    <motion.div
      ...
      className={`${
        expanded
          ? "absolute top-0 bottom-0 right-0 left-0 m-[auto_auto] w-[calc(100vw-100px)] h-[calc(100vh-100px)] z-100"
          : "relative w-full h-full cursor-pointer"
      } pt-6 px-10 pb-6 rounded-2xl bg-base-400 border max-w-[800px] border-[var(--initial-border-color)]  hover:border-[var(--border-color)] duration-150 flex flex-col gap-2 group `}
    >
      <div className="pl-2 flex items-end"> 
        <h4 className={`${expanded ? "text-4xl" : "text-2xl"} font-bold line-clamp-3 capitalize`}>
          {name}
        </h4>
        {expanded && (
      ...
      <div className="flex justify-between items-start">
        {description ? (
          <p
            ...
            } text-xl font-light mt-2 text-[var(--card-textcolor)]`}
          >
            {description}
          </p>
        ) : (
