...
const getFilterOptionsForType = (colType: string) =>
  FILTER_OPTIONS[colType] ?? FILTER_OPTIONS.text;
...
export default function FilterDropdown() {
  ...
  const appliedCount = Object.keys(columnStates).filter(
    k => columnStates[k]?.filter && (columnStates[k].filter === "empty" || columnStates[k].filter === "non_empty" || !!columnStates[k].filterValue)
  ).length;
  ...
  return (
    <div className={`dropdown ${filterDropdownOpen ? "dropdown-open" : ""}`}>
        ...
        onClick={() => {
          if (filterDropdownOpen) {
            ...
            const filterDraft = Object.entries(columnStates)
              .filter(([, s]) => s?.filter && (s.filter === "empty" || s.filter === "non_empty" || !!s.filterValue))
              .map(([columnKey, s]) => ({
                columnKey,
                filter: s.filter!,
      ...
      <div
        tabIndex={0}
        className="dropdown-content menu bg-base-200 rounded-box z-1 p-4 shadow-lg min-w-[420px] mt-2"
      >
        <div className="space-y-2">
          {filterDraft.length > 0 ? (
                ...
                <div key={index} className="flex gap-2 items-center">
                  <select
                    ...
                    onChange={e => {
                      ...
                      const opts = newCol ? getFilterOptionsForType(newCol.type) : FILTER_OPTIONS.text;
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      useAdminStore.setState(state => ({
                        filterDraft: state.filterDraft.map((r, i) =>
                          i === index ? { ...r, filter: val, filterValue: needsValue(val) ? r.filterValue : "" } : r
                        ),
                      }));
                    }}
                  >
                  ...
                  {showValue && (
                    <input
                      ...
                      onChange={e =>
                        useAdminStore.setState(state => ({
                          filterDraft: state.filterDraft.map((r, i) =>
                            i === index ? { ...r, filterValue: e.target.value } : r
                          ),
                        }))
                      }
                      placeholder="Value"
            ...
            <p className="text-center text-base-content/60">No filters applied</p>
          )}
        </div>
        <div className="flex gap-2 mt-4 pt-2 border-t border-base-content/20">
          ...
          <button
            ...
            onClick={() => {
              ...
              const columnStates: Record<string, { sort: null; filter: FilterOperator; filterValue: string }> = {};
