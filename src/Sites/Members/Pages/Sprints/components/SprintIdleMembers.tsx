...
export default function SprintIdleMembers({ members, tasks, teamTab }: SprintIdleMembersProps) {
  ...
  const idle = members
    .filter(member => !busyIds.has(member.id))
    .filter(member => {
      if (teamTab === "ALL") return true;
      return memberTeamKeys(member).includes(teamTab);
    })
      ...
      <div className="mt-4">
        {idle.length === 0 ? (
          <p className="m-0 text-sm text-(--obs-text-muted)">
            Every listed board member has at least one to-do, in-progress, or pending-review task
            {teamTab === "ALL" ? "" : ` on ${teamLabel(teamTab)}`}.
          </p>
        ) : (
          <>
            <p className="mt-0 mb-3 text-sm leading-6 text-[#fca5a5]">
              Not assigned to any to-do, in-progress, or pending-review card this sprint. Completed
              or dropped work does not count.
            </p>
